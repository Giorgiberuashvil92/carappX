...
export default function AccessoriesScreen() {
  ...
  const loadStores = useCallback(async () => {
    try {
      setLoading(true);
      const response = await addItemApi.getStores({ type: 'აქსესუარები' });
      if (response.success && response.data) {
        // Filter only accessory stores
        const accessoryStores = response.data.filter((store: any) => 
  ...
  const convertStoreToDetailItem = (store: any): DetailItem => {
    ...
    return {
      ...
      title: store.name,
      name: store.name,
      description: store.description || `${store.name} - ხარისხიანი აქსესუარები თქვენი ავტომობილისთვის`,
      image: mainImage,
      type: 'store',
      ...
      <SafeAreaView edges={['top']} style={styles.header}>
        <View style={styles.headerContent}>
          ...
          <View style={styles.headerCenter}>
            ...
            <Text style={styles.title}>აქსესუარების მარკეტპლეისი</Text>
          </View>
        ...
        <View style={styles.searchContainer}>
          ...
          <TextInput
            ...
            placeholder="მოძებნე აქსესუარი ან მაღაზია..."
            value={searchQuery}
        ...
        {loading ? (
          ...
          <>
            ...
            <View style={styles.section}>
              <TouchableOpacity
                activeOpacity={0.8}
                onPress={() => router.push('/map?type=აქსესუარები')}
              >
                <LinearGradient
              ...
              {filteredStores.length === 0 ? (
                <View style={styles.emptyState}>
                  <View style={styles.emptyIconContainer}>
                    <Ionicons name="grid-outline" size={64} color="#F97316" />
                  </View>
                    ...
                    >
                      ...
                      <View style={styles.cardContent}>
                        ...
                        <View style={styles.cardFooter}>
                          ...
                          >
                            <Ionicons name="call" size={12} color="#F97316" />
                          </TouchableOpacity>
                        </View>
                      </View>
                    </TouchableOpacity>
